...
class CrmSuggestionSchemaMigrationTest {
    ...
    fun migration32To33MakesOpportunityNullableAddsContactAndPreservesRows() {
        ...
        helper.createDatabase(name, 32).use { db ->
            ...
            db.execSQL(
                "INSERT INTO crm_opportunities (opportunityId, title, accountNameSnapshot, primaryContactId, " +
                    "stage, status, currencyCode, probabilityPercent, sourceType, createdAtEpochMs, updatedAtEpochMs) " +
                    "VALUES ('opportunity-1', '机会', '客户', 'contact-1', 'QUALIFIED', 'OPEN', 'CNY', 50, 'MANUAL', 1, 1)",
            )
            db.execSQL(
                "INSERT INTO crm_agent_suggestions " +
                    "(suggestionId, opportunityId, suggestionType, title, summary, rationale, evidenceRefsJson, " +
                    "confidence, proposedActionJson, status, createdAtEpochMs, updatedAtEpochMs) " +
        ...
        helper.runMigrationsAndValidate(name, 33, true, AgentDatabase.MIGRATION_32_33).use { db ->
            ...
            db.execSQL(
                "INSERT INTO crm_agent_suggestions " +
                    "(suggestionId, opportunityId, contactId, suggestionType, title, summary, rationale, " +
                    "evidenceRefsJson, confidence, proposedActionJson, status, createdAtEpochMs, updatedAtEpochMs) " +
