...
/** 变更撤销的应用入口(由 runtime.governance 的 ChangeUndoCoordinator 实现,data 只依赖接口)。 */
@Singleton
interface ChangeUndoApplier {
    suspend fun undoVisible(changeId: String, nowEpochMs: Long): Boolean
    suspend fun undoForRun(changeId: String, runId: String, nowEpochMs: Long): Boolean
}

class AutoWriteRepository @Inject internal constructor(
    private val database: AgentDatabase,
    @ApplicationContext private val context: Context,
    private val undoApplier: ChangeUndoApplier,
) {
    ...
    private suspend fun undoAndMarkCorrected(changeId: String, nowEpochMs: Long): Boolean {
        ...
        val undone = if (database.changeLogDao().findAutoWriteReceipt(changeId) != null) {
            undoApplier.undoVisible(changeId, nowEpochMs)
        } else {
            change.runtimeRunId?.let { undoApplier.undoForRun(changeId, it, nowEpochMs) } ?: false
        }
