...
| 10.3c | SSE 末帧无换行被误报超大帧 | ✅ | 功能不可用 | 合法 SSE 在 EOF 前可省略末尾换行；旧实现用 `readUtf8LineStrict`，会把短小末帧的正常 EOF 当成 `PROVIDER_FRAME_TOO_LARGE`。现先在 64KiB 界内寻找换行，无换行时只读取已确认不超限的剩余字节，仍保持单帧上限 | 本提交 `fix(10.3c)` | `ProviderModuleTest.cleanEofWithoutTrailingNewlineStillDecodesTheLastSseFrame`（红→绿） + 既有超大帧 fail-closed 回归 |
| 10.3d | HTTP 200 SSE 内错误对象不再伪装为空成功 | ✅ | 功能不可用 | Provider 可在成功 HTTP 流内返回顶层 `error` 对象；旧解码器只找 `choices`，随后在 DONE 生成空 Final，丢失鉴权/限流等真实原因。现流入口先映射固定失败码与脱敏 requestId，未知错误 fail-closed 为 `PROVIDER_REJECTED`，不暴露外部 message | 本提交 `fix(10.3d)` | `ProviderModuleTest.providerErrorInsideSuccessfulSseResponseIsNeverTreatedAsAnEmptySuccess`（红→绿） |
| 10.3e | 弱网断线时完整工具调用不会被竞态丢失 | ✅ | 功能不可用 | 完整工具帧后若连接断开，旧实现先 emit 工具与 Final、再抛 IOException；`flowOn` 的通道关闭可能竞态丢掉刚发事件，导致同一响应随机执行或失败。现仅在工具参数完整可解析时正常收口，半截文本仍按传输失败处理 | `696f9dd` | `OpenAiCompatibleProviderAdapterTest.ioFailureAfterCompleteToolFrameRecoversPendingToolCallWithoutRacingTheFailure` + `ioFailureAfterPartialTextDoesNotPretendTheResponseCompleted`（debug/release 连续 10 轮 + 完整 check） |
...
| 16.20 | 异常命令进入死信而非永久重放 | ✅ | 功能不可用 | 已 claim 的命令若在解析/状态处理时抛出意外异常，runner 只返回 `FAILED`，命令仍为 `CLAIMED` 并在每次租约到期后重复失败。现保留取消语义，其余逃逸异常在同一租约下以固定脱敏原因终结命令；`Start` 命令同时把尚未启动的 run 原子转为 `FAILED_FINAL` | 本提交 `fix(16.20)` | `RuntimeInputProcessorTest.malformedClaimedCommandIsDeadLetteredInsteadOfRetriedForever`（SM-W7023：损坏已接收命令后只处理一次，命令 `FAILED`、run `FAILED_FINAL`，队列随后空闲） |
| 16.21 | 观察期等待模型时保持租约 | ✅ | 数据一致性 | 观察流原先只在收到 Provider 事件时续租，静默等待超过租约后可被恢复 owner 抢占并重复第二轮模型请求。现观察 Provider 流全程运行同 fencing epoch 心跳，直到流结束或取消 | 本提交 `fix(16.21)` | `RuntimeInputProcessorTest.stalledObservationRenewsLeaseUntilItsProviderStreamCompletes`（SM-W7023：100ms 租约、静默 250ms 后第二 owner 仍不可 claim，释放后多工具链完成） |
...
| 17.13 | 自动召回上下文不会裸发原始凭据 | ✅ | 隐私/安全 | 自动召回的记忆、联系人备注或历史消息若含无字段名的 `sk-`、`ghp_` 等凭据，旧策略只识别 Bearer 和 `api_key=` 形式，可能原样进入模型请求。现出境总闸额外识别高置信厂商前缀及 48–96 位混合型不透明凭据；用户当前主动输入的既有策略不变 | 本提交 `fix(17.13)` | `OutboundDataPolicyTest.automaticContextRedactsRawHighConfidenceCredentials`（红→绿） |
