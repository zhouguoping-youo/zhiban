...
| 1.17 | 语音记录日程结果 | ✅已修复 | “更新进展”里的麦克风直接启动 Google/系统语音识别面板，国内无 Google 服务的设备无法可靠使用 | 新建日程→更新进展→点“语音填写结果”，观察是否跳出知伴以及转写能否回填 | 功能不可用 | `ScheduleCompletionDialog` 直接发送 `RecognizerIntent.ACTION_RECOGNIZE_SPEECH`，绕过知伴已有的阶跃 ASR、出境策略和失败恢复 | 本提交 `fix(1.17)` | `ScheduleOutcomeVoicePolicyTest`；`ScheduleProgressActionsTest.voiceResultUsesInAppProviderStateInsteadOfLaunchingSystemRecognizer`；`ScheduleOutcomeVoiceRecordingTest.availableProviderRecordsInAppThenWritesTranscriptionIntoOutcomeField` | 2026-08-15 SM-W7023：真实 App 路径创建并清理 `VOICE_TEST`，点击麦克风后前台仍为知伴且未出现 Google；未配置模型时原弹窗内提示恢复方法。设备测试用可用 Provider 完成 600ms 本地录音→受控转写回调→结果回填，1/1 通过；完整日历进展 UI 3/3 通过 |
